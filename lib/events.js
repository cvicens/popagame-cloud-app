...
function route() {
  ...
  router.get('/:country/:city/:username', function(req, res) {
    var country = req.params.country;
    var city = req.params.city;
    var username = req.params.username;
    console.log('Find event by country', country, 'city', city, 'username', username);
    if (typeof country === 'undefined' || country == '' ||
        typeof city === 'undefined' || city == '' ||
    ...
    searchEvents(filter)
    .then(function (data) {
      console.log('searchEvents => data', data);
      console.log('searchEvents => eventId', data);

      Promise.all(data.map((event) => getQuizStatusById(event.id, username)))
      .then((status) => {
        res.status(200).json({ok: true, data: data, status: status});
      })
      .catch((err) => {
        res.status(200).json({ok: true, data: data});
      });
