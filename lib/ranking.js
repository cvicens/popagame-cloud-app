...
function route() {
  ...
  router.get('/new/:eventId', function(req, res) {
    var eventId = req.params.eventId;
    console.log('Fetch ranking by eventId ', eventId);
    if (typeof eventId === 'undefined' || eventId == '') {
      res.status(400).json([]);
    ...
    then(function (data) {
      ...
      data.forEach(element => {
        if (typeof _data[element.username] === 'undefined') {
          _data[element.username] = { 
        ...
        console.log(JSON.stringify(_data[element.username]));

        if (element.result === 'CORRECT') {
          console.log('ranking[element.username]', _data[element.username]);
          _data[element.username].points = _data[element.username].points + element.points;
        }
      });
      ...
      for (const key in _data) {
        if (_data.hasOwnProperty(key)) {
          ranking.push({
            eventId: eventId, 
            userId: _data[key].userId, username: _data[key].username, 
            firstName: _data[key].firstName, lastName: _data[key].lastName,
            points: _data[key].points});
        }
      }


      res.status(200).json({ok: true, data: ranking.sort((obj1, obj2) => obj2.points - obj1.points) });
    })
    .catch(function (err) {
      res.status(500).json({ok: false, msg: err})
    });
